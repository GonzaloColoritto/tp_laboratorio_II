...
namespace Entidades
{
    public delegate void CarganInfo();
    public class ArchivoXml : IArchivador<List<Premio>,List<Electrodomestico>>
    {
        private readonly string path;
        public event CarganInfo cargandoInfo;
        public event CarganInfo infoCargada;
        ...
        public void LeerPremios()
        {
            if (File.Exists(this.path))
            {
                using (XmlTextReader xmlTextReader = new XmlTextReader(path))
                {
                    ...
                    List<Premio> listaAuxPremios = new List<Premio>();


                    LocalParaLaCasa.ListaPremios = (List<Premio>)serializer.Deserialize(xmlTextReader);
        ...
        /// </summary>
        /// <returns>Retornara la lista leida</returns>
        public void LeerElectrodomesticos()
        {
            if (File.Exists(this.path))
            {
                using (XmlTextReader xmlTextReader = new XmlTextReader(path))
                {
                    ...
                    LocalParaLaCasa.Electrodomesticos = (List<Electrodomestico>)serializer.Deserialize(xmlTextReader);
                }
            }
            else
            {
